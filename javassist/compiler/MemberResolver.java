...
public class MemberResolver implements TokenId {
   ...
   private MemberResolver.Method lookupMethod(CtClass clazz, String methodName, int[] argTypes, int[] argDims, String[] argClassNames, boolean onlyExact) throws CompileError {
               ...
               do {
                  do {
                     do {
                        ...
                        minfo = (MethodInfo)var10.next();
                     } while(!minfo.getName().equals(methodName));
                  } while((minfo.getAccessFlags() & 64) != 0);

                  res = this.compareSignature(minfo.getDescriptor(), argTypes, argDims, argClassNames);
               } while(res == -1);

               r = new MemberResolver.Method(clazz, minfo, res);
               if (res == 0) {
                  return r;
               }
            } while(maybe != null && maybe.notmatch <= res);
      ...
      try {
         ...
         CtClass[] var24 = ifs;
         int var26 = ifs.length;

         for(int var14 = 0; var14 < var26; ++var14) {
            CtClass intf = var24[var14];
            MemberResolver.Method r = this.lookupMethod(intf, methodName, argTypes, argDims, argClassNames, onlyExact);
            if (r != null) {
               return r;
            }
         }
