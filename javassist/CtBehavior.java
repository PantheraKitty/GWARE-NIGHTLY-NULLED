   ...
   public void insertAfter(String src, boolean asFinally, boolean redundant) throws CannotCompileException {
      ...
      if (ca == null) {
         ...
         try {
               ...
               if (c == 176 || c == 172 || c == 174 || c == 173 || c == 175 || c == 177) {
                  if (redundant) {
                     ...
                     if (noReturn) {
                        ...
                        adviceLen2 = jvc.recordParams(this.getParameterTypes(), Modifier.isStatic(this.getModifiers()));
                        jvc.recordParamNames(ca, adviceLen2);
                        retVarNo = jvc.recordReturnType(rtype, true);
                        jvc.recordLocalVariables(ca, 0);
                     }

                     adviceLen2 = this.insertAfterAdvice(bcode, jvc, src, pool, rtype, retVarNo);
            ...
            throw new CannotCompileException(var27);
         } catch (CompileError var28) {
            throw new CannotCompileException(var28);
         } catch (BadBytecode var29) {
            throw new CannotCompileException(var29);
         }
      }
   }
